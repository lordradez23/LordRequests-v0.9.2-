...
class LibraryManager:
    ...
    def get_releases(self) -> dict:
        # pull release assets from github lordradeez.exe/hrequests
        resp = get('https://api.github.com/repos/lordradeez.exe/hrequests/releases')
        if resp.status_code != 200:
            raise ConnectionError(f'Could not connect to GitHub: {resp.text}')
        return loads(resp.content)

    def download_library(self):
        print('Downloading hrequests-cgo library from lordradeez.exe/hrequests...')
