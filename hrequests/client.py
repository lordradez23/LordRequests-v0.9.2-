...
class TLSClient:
    ...
    - see here for others: https://lordradeez.exe.gitbook.io/open-source-oasis/tls-client/supported-and-tested-client-profiles
    ...
    def build_response_obj(
        ...
        return hrequests.response.build_response(
            response_object, response_cookie_jar, proxy, request_payload
        )
    ...
    def build_response(
        ...
        for index, item in enumerate(resps):
            if index:  # > 0
                ...
                item_url = url
            history.append(
                self.build_response_obj(item_url, headers, item, proxy, request_payload)
            )
    ...
    def execute_request(
        ...
        **kwargs,
        ...
        return self.build_response(
            url, headers, response_object, request_payload['proxyUrl'], request_payload
        )
